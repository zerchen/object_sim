...
for object_name in os.listdir(target_category):
    ...
    with open(xml_file_path, 'w') as f:
        ...
        f.writelines(f"        <body name=\"{object_name}\" pos=\"0.0 0.0 {z_str}\">\n")
        ...
        f.writelines(f"            <geom name=\"{object_name}_visual\" class=\"object_visual\" mesh=\"{object_name}\" />\n")
        for idx in range(num_contact_files):
            f.writelines(f"            <geom name=\"{object_name}_contact{idx}\" class=\"object_contact\" mesh=\"contact{idx}\" />\n")
        f.writelines(f"        </body>\n")
        f.writelines(f"    </worldbody>\n")
