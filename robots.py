...
from physics import ROBOT_CONFIGS
...
class BaseRobot(MjModel):
    def __init__(self, robot_name, limp=False, vis=False):
        ...
        base_model = mjcf.from_path('robots/{}/base.xml'.format(robot_name))
        self._attach_mocap = bool(limp)
        self._vis = vis
        if not self._attach_mocap:
            # adds motors to actuated model
            actuators = mjcf.from_path('robots/{}/actuators.xml'.format(robot_name))
            for a in actuators.actuator.all_children():
                a_data = {}
                if a.tag == 'general':
                    keys = ['name', 'class', 'joint', 'ctrllimited', 'ctrlrange', 'biastype', 'gainprm', 'biasprm', 'forcerange']
                elif a.tag == 'position':
    ...
    def _post_attach(self, env_mjfc):
        if self._attach_mocap:
            ...
            with open('robots/{}/mocaps.txt'.format(self._robot_name), 'r') as f:
            ...
            for i, p in enumerate(mocap_locations):
                ...
                env_mjfc.equality.add('connect', body1=mocap_name, body2=robot_body, anchor=[0,0,0], solimp=[0.9,0.95,0.001], solref=[0.02,1])
