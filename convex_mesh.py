...
if __name__ == '__main__':
    ...
    # for hoi4d
    # classes = ['Mug']
    # hoi4d_model_path = '/home/zerui/workspace/code/dex_sim/datasets/hoi4d/data/HOI4D_CAD_Model_for_release/rigid'
    # for obj_class in classes:
        # for filename in os.listdir(os.path.join(hoi4d_model_path, obj_class)):
            # instance_id = filename.split('.')[0].zfill(4)
            # object_path = f'/home/zerui/workspace/code/object_sim/hoi4d/{instance_id}_{obj_class}'
            # os.makedirs(object_path, exist_ok=True)
            # object_mesh = trimesh.load(os.path.join(hoi4d_model_path, obj_class, filename)).simplify_quadric_decimation(5000)
            # object_mesh = object_mesh.apply_translation(-object_mesh.centroid)
            # object_mesh.export(os.path.join(object_path, 'textured.obj'))
            # create_collision_mesh(os.path.join(object_path, 'textured.obj'), object_path)
